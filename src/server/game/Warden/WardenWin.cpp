...
void WardenWin::Init(WorldSession* session, BigNumber* k)
{
    ...
    //SF_LOG_DEBUG("warden", "Module ID: %s", ByteArrayToHexStr(_module->Id, 32).c_str());
    RequestModule();
}
...
void WardenWin::RequestStaticData()
{
    ...
    for (std::list<uint16>::iterator itr = _currentChecks.begin(); itr != _currentChecks.end(); ++itr)
    {
        ...
        switch (type)
        {
            ...
            case PAGE_CHECK_B:
            {
                buff.append(wd->Data.AsByteArray().get(), wd->Data.GetNumBytes());
                buff << uint32(wd->Address);
                buff << uint8(wd->Length);
                break;
            }
...
float WardenWin::GetServerSpeed(Unit * obj, UnitMoveType mtype)
{
    ...
    case MOVE_FLIGHT:
    {
        if (obj->GetTypeId() == TypeID::TYPEID_UNIT && obj->IsControlledByPlayer()) // not sure if good for pet
        {
        ...
        //if (AuraEffect* aura = obj->GetAuraEffect(SPELL_AURA_MOD_INCREASE_SPEED, SPELLFAMILY_GENERIC, 1797, 0))
        //{
        //    float non_stack_bonus2 = 1.0f;
        //    non_stack_bonus2 += aura->GetAmount() / 100.0f;
    ...
    case MOVE_FLIGHT:
    {
        ...
        if (obj->GetTypeId() == TypeID::TYPEID_UNIT)
...
bool WardenWin::IsAllowFlyingOnVehicles(Unit * vb)
{
    ...
    if (vb->GetTypeId() == TypeID::TYPEID_UNIT)
    {
