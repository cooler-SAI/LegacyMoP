...
int32 SpellScript::GetAbsorbedDamage()
{
    if (!IsInTargetHook())
    {
        SF_LOG_ERROR("scripts", "Script: `%s` Spell: `%u`: function SpellScript::GetAbsorbedDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
