        ...
        class spell_warr_shield_barrier_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEff*/, int32 & amount, bool & /*canBeRecalculated*/)
            {
                if (GetCaster())
                {
                    ...
                    int32 AP = int32(GetCaster()->GetTotalAttackPowerValue(WeaponAttackType::BASE_ATTACK));
        ...
        class spell_warr_storm_bolt_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        if (unitTarget->GetTypeId() == TypeID::TYPEID_UNIT && unitTarget->IsImmunedToSpellEffect(sSpellMgr->GetSpellInfo(WARRIOR_SPELL_STORM_BOLT_STUN), 0))
...
class spell_warr_staggering_shout : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_staggering_shout_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(Skyfire::UnitAuraTypeCheck(false, SPELL_AURA_MOD_DECREASE_SPEED));
            }
...
class spell_warr_heroic_leap_damage : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_heroic_leap_damage_SpellScript : public SpellScript
        {
            ...
            {
                if (Unit* caster = GetCaster())
                    SetHitDamage(int32(caster->GetTotalAttackPowerValue(WeaponAttackType::BASE_ATTACK) * 0.5f));
            }
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        if (SpellInfo const* l_Execute = sSpellMgr->GetSpellInfo(5308))
                        {
                            for (SpellPowerEntry const* l_Power : l_Execute->SpellPowers)
                            {
                                ...
                                {
                                    int32 l_Cost = int32(l_Power->manaCost * -1.0f);
        ...
        class spell_warr_glyph_of_gag_order_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (target->GetTypeId() != TypeID::TYPEID_PLAYER && caster->HasAura(WARRIOR_GLYPH_OF_GAG_ORDER))
                        {
...
class spell_warr_enrage : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_enrage_AuraScript : public AuraScript
        {
            ...
            void CalculateAmount(AuraEffect const* /*aurEffect*/, int32& amount, bool& canBeRecalculated)
            {
                Player* player = GetCaster()->ToPlayer();
                amount = 10;
                if (player && player->GetSpecializationId(player->GetActiveSpec()) == SPEC_WARRIOR_FURY && GetCaster()->HasAura(76856))
                    amount += int32(GetCaster()->GetFloatValue(PLAYER_FIELD_MASTERY) * 1.4f);
